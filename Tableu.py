...
def adjacentLocations(position):
	'''
        Revenir pièces adjacentes à la pièce en position
	@param position: Indice de la pièce
	'''
	adjacent = [
		[1, 3],
		[0, 2, 9],
...
def checkMillFormation(position, board, player):
	'''	
        Retourne vrai s'il y a un moulin en position pour joueur sur un tableau donné
	@param position: L'indice de la position que nous vérifions
	@param board: La liste du tableau actuel
	@param player: Représentation en chaîne de la couleur de la plaque
	'''
...
def isCloseMill(position, board):
	'''	
        Retourne vrai si un joueur a un moulin sur position
	@param position: L'indice de la position que nous vérifions
	@param board: La liste du tableau actuel
	'''
